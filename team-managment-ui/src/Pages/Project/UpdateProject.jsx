...
const UpdateProject = ({ open, onClose, show, project, user }) => {
  const dispatch = useDispatch();
  const CreatedById = user.user && user.user.userId;
  const getImage = (imagePath) => {
    return createImagePath(imagePath);
  };
  const [AssignedTo, setAssignedTo] = useState(project && project.assignedTo);
  ...
  const [selectedEmployee, setSelectedEmployee] = useState(
    project.teamEmployees && project.teamEmployees
  );
  const [selectedTeam, setSelectedTeam] = useState(
    project.teamProjects && project.teamProjects[0]
  );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await updateProject(projectPost);
  ...
  return (
    <>
      ...
      <div style={MODAL_STAYL}>
        <div style={TOPER}>
          <div style={COLU}>
            <h1 style={HTITLE}>Update Project</h1>
            <div style={LINE}></div>
          </div>
            ...
            <div style={ONECOLEE}>
              <div style={INPUTWRAPS}>
                ...
                <input
                  ...
                  id="AssignedDate"
                  name="AssignedDate"
                  value={AssignedDate && AssignedDate.split("T")[0]}
                  type="date"
